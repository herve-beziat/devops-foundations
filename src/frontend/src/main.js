    ...
    <div class="grid">
      ...
      <div class="card card-services">
        ...
        <div class="card-data">
          ...
          <div class="data-row">
            <a class="service-link" href="https://api.localhost" target="_blank">api.localhost</a>
            <span class="service-desc">{"message":"welcome"}</span>
          </div>
          <div class="data-row">
...
function setCard(id, status, data) {
  ...
  card.className = 'card ' + (status === 'ok' ? 'status-ok' : status === 'error' ? 'status-error' : 'status-loading')
  badge.className = 'badge ' + (status === 'ok' ? 'ok' : status === 'error' ? 'error' : 'loading')
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    if (el) {
      ...
      el.className = 'data-value' + (status === 'error' ? ' error' : key === 'visits' ? ' highlight' : '')
    }
  })
}
...
async function checkBackend() {
  try {
    ...
    setCard('backend', 'error', { status: 'unreachable', service: '—', version: '—' })
    return false
  }
}
...
async function checkDb() {
  try {
    ...
    setCard('db', 'error', { status: 'unreachable', name: '—', timestamp: '—' })
    return false
  }
}
...
async function checkCache() {
  try {
    ...
    setCard('cache', 'error', { status: 'unreachable', engine: '—', visits: '—' })
    return false
  }
}
